...
module Blacklight::SearchContext
  ...
  def add_to_search_history(search)
    ...
    session[:history] = session[:history].slice(0, blacklight_config.search_history_window) if session[:history].length > blacklight_config.search_history_window
  end

  # A list of query parameters that should not be persisted for a search
  def blocklisted_search_session_params
  ...
  def setup_next_and_previous_documents
    if search_session['counter'] && current_search_session
      ...
      { prev: documents.first, next: documents.last, counter: search_session['counter'] }
    end
