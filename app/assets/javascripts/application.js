...
$(document).on('turbolinks:load', function () {
	// Receiving the data:
	let fullTextSearchSelected = new RegExp('[?&]search_field=([^&#]*)').exec(
		window.location.href,
	);
	fullTextSearchSelected =
		fullTextSearchSelected && fullTextSearchSelected[1] === 'fulltext_tesim';
...
$(document).on('turbolinks:load', function () {
	...
	$('.fulltext-button').click(function () {
		...
		fulltextTranscription.toggle(function (i, text) {
			$(this).is(':visible')
				? fulltext_button.text('Hide Full Text')
				: fulltext_button.text('Show Full Text');
		});
...
const fulltext = () => {
	...
	if ($('.fulltext-button').length) {
		...
		child_oids_array.forEach(async child_oid => {
			...
			return fulltextTranscription.append(
				`<span class='${pageWidth}'>${transcription}</span>`,
			);
		});
	} else {
		return;
...
const getFulltext = async child_oid => {
	const result = await $.ajax({
		...
		url: `/annotation/oid/${$(
			'#parent-oid',
		).text()}/canvas/${child_oid}/fulltext`,
		data: {
			oid: $('#parent-oid').text(),
